...
const splitExcerpt = function () {
  return function (file) {
    if (!isUtf8(file.contents)) return
    let str = file.contents.toString()
    if (rFindExcerpt.test(str)) {
      const split = str.split(rFindExcerpt)
      str = `<div class="excerpt">${split[0]}</div><div class="more">${split[3]}</div>`
    }
